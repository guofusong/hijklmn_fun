(function(){
	...
	fun.start = function(){
		...
		// 初始化富文本框
		fun.initCKEditor(sound);
		
		if(sound){
			
			fun.fillData(sound);
			
			if(sound['soundTopPic']){
				$("#picPreview").attr("src",imageUrl +'/'+ sound['soundTopPic']);
			}
			
			if(sound['soundUrl']){
				$("#soundPlay").attr("src",soundUrl +'/'+ sound['soundUrl']);
			}
		...
		$("#soundForm").change(function(){
			var file = document.getElementById('soundForm').files[0];
			var fileReader = new FileReader();
            fileReader.onloadend = function () {
                if (fileReader.readyState == fileReader.DONE) {
                    document.getElementById('soundPlay').setAttribute('src', fileReader.result);
                }
            }
            fileReader.readAsDataURL(file);
		});
	...
	fun.uploadSound = function(){
	
		var file = document.getElementById('soundForm').files[0];
		
		if(file == null || file == ''){
			hijklmn.notice.current("请选择声音");
			return;
		}
	...
	fun.initCKEditor = function(sound){
		...
		ClassicEditor.create( document.querySelector( '#editor' ),
	        ...
	        editor = newEditor;
	        if(sound){
	        	newEditor.setData(sound['content']);
	        }
	    } )
        .catch( error => {
